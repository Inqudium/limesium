 ...
 *
 * Injectable for the same reason as [NanoTimeSource] and [CorrelationIdGenerator]: the fingerprint's
 * shape is a policy the host may own - a keyed HMAC for a compliance regime that forbids unkeyed hashes,
 * a fixed `***` for a host that wants no correlation at all - and both twins take the bean through their
 * auto-configuration (`@ConditionalOnMissingBean`), so one host bean masks the servlet line and the
 * reactive line alike.
 *
