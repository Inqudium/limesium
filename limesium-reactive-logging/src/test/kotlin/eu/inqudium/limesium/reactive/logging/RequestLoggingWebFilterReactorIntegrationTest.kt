 ...
 * majority consumer configuration, where the optional coroutine dependencies are absent (assessment
 * finding 6, 2026-08-22T16-35-46 analysis). Proves the Reactor-specific `Mono.defer`/`doFinally`
 ...
 * static configuration - see the Spring Boot test isolation caveat in CLAUDE.md. The Reactor variant's
 * initializer writes the accessors into the JVM-global ContextRegistry at context start; the class-level
 * guard takes them out again afterwards (finding 4 of CODE_ANALYSIS-2026-08-22T23-32-09.md).
 */
@SpringBootTest(
...
class RequestLoggingWebFilterReactorIntegrationTest {
    ...
    fun `should log the status and header a later commit action applies on a real error response`() {
        ...
        //   2026-08-22T20-06-45 analysis) - a downstream WebFilter registers a beforeCommit action that
