...
`ClusterFuzzLite batch fuzzing` workflow. A finding fails the run and attaches
the reproducing input as a workflow artifact; turn such an input into a
regression test before fixing. New parsing/bounding surface should bring a
fuzz target stating its invariants, like the existing ones do.

To reproduce locally (requires Docker):

```bash
docker build -t limesium-fuzz -f .clusterfuzzlite/Dockerfile .
docker run --rm -e FUZZING_LANGUAGE=jvm -e SANITIZER=address \
  -v "$PWD/build-out:/out" limesium-fuzz compile
docker run --rm -v "$PWD/build-out:/out" gcr.io/oss-fuzz-base/base-runner \
  run_fuzzer TraceparentFuzzer -runs=100000
```
