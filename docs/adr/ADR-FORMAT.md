...
or [ADR-0003](ADR-0003-limesium-common-inlined-by-shade.md)) and
mirror its shape.
...
- `NNNN` is a zero-padded, four-digit sequence number that never gets
  ...
  [ADR-0001](ADR-0001-fuzz-workflow-is-the-fuzzing-signal.md): the number is the
...
- The title is a concise noun phrase or a one-line statement naming
  the decision (`Body logging is a mode per direction, gated by the outcome`,
  `The default correlation id is a counting id, not a UUID`), not a paragraph.
...
- **Cross-link** related ADRs with relative links
  (`[ADR-0003](ADR-0003-limesium-common-inlined-by-shade.md)`), and
...
- **Citing from code:** an in-code comment, a POM comment, or an
  ...
  decision trail, not the normative statement (`CONTRIBUTING.md`,
  "One normative source per contract statement").
