...
- [x] 2. [UNIT — querschnittlich; repräsentativ `./limesium-reactive-logging/src/main/kotlin/eu/inqudium/limesium/reactive/logging/ExchangeLifecycle.kt:128-175` und `./limesium-servlet-logging/src/main/kotlin/eu/inqudium/limesium/servlet/logging/RequestLoggingFilter.kt:176-291`] {Low} {Confidence: medium} {Under-Engineering / Coupling & Cohesion} Fail-open-Guard-Boilerplate wiederholt sich weit jenseits der Dreierregel (siehe Muster P1)
  - **Status: FIXED (2026-08-31)** — Neuer `failOpen(onInterrupted, onFailure) { }`-Helfer neben `reportQuietly` in `limesium-common`; die sechs vollständig verschluckenden Guard-Stellen (beide Emitter: Arrival + Emission; Lifecycle: Commit-Callback-Körper + Registrierung) nutzen ihn — alle sechs wörtlichen Interrupt-Restore-Blöcke außerhalb von `onTerminal` sind damit zentralisiert. Die Standorte mit abweichender Semantik (Rethrow, Ergebniswert, Completion nach Fehler) behalten wie im Finding vorgesehen ihr explizites try/catch; der Helfer dokumentiert das.
...
- [x] 3. [UNIT — Dokumentationsgewebe der Produktionsmodule, repräsentativ `./limesium-servlet-logging/src/main/kotlin/eu/inqudium/limesium/servlet/logging/RequestLoggingFilter.kt:23-107`] {Low} {Confidence: medium} {Consistency / Maintainability} Die Prosa-Masse hat messbare, wiederkehrende Drift-Kosten
  - **Status: ADDRESSED (2026-08-31)** — Kein Code-Fix; die im Finding benannte Gegenstrategie ist die bereits etablierte und hiermit bestätigte Praxis: datierte, auflösbare Provenance (Comment-Audit CA-07), generierte statt gepflegte Evidenz (Test-Katalog/Coverage), ausführbare Verträge vor beschreibender Wiederholung (Literal-Pins, Referenz-YAML; mit Finding 1 dieser Review zusätzlich eine Prosa-Pflichtstelle weniger, da die Maskierungs-Doku nur noch einmal existiert). Wiedervorlage implizit: jeder künftige Analysebericht misst die Drift erneut.
