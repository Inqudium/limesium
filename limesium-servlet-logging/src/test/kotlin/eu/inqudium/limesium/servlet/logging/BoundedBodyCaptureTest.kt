...
class BoundedBodyCaptureTest {
    ...
    inner class `Truncation at a character boundary` {
        ...
        fun `should still replace malformed bytes inside the prefix`() {
            ...
            //   malformed bytes inside the prefix are the caller's problem and must stay visible as
            //   such.
    ...
    inner class `Read state` {
        ...
        fun `should move to partial on start and to complete on completion, never backwards`() {
            // What is tested: the read-state transitions - partial on markStarted, complete on
            //   markCompleted, and a later markStarted.
            // Success criteria: PARTIAL, then COMPLETE, and COMPLETE again after the late start.
            // Why it matters: the state is the `state` tag of the read counter; a second
            //   getInputStream call regressing complete to partial would count a fully read body as
            //   abandoned.
            // Given: a fresh capture
            val capture = BoundedBodyCapture(8)
