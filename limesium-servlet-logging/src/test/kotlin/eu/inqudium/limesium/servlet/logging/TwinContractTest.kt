...
class TwinContractTest {
    ...
    fun `should pin the MDC keys to the literal twin contract`() {
        // What is tested: the MdcKeys and TraceMdcKeys literals of the servlet stack - including
        //   the bridge's spanId key.
        // Success criteria: the three endpoint_* keys, traceId, parentSpanId and spanId spelled
        //   exactly so.
    ...
    fun `should pin the shared outcome vocabulary plus this stack's own disposition`() {
        // What is tested: the outcome literals of the servlet stack - the shared success, rejected
        //   and failure plus timeout.
        ...
        assertThat(EndpointLoggingMetrics.OUTCOME_TIMEOUT).isEqualTo("timeout")
    }
    ...
    fun `should pin the exchange and arrival message format to the literal twin contract`() {
        ...
        try {
            // When: one successful exchange, destroyed by the container
