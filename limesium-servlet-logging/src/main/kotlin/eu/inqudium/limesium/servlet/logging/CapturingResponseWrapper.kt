 ...
 * those alone logged discarded pre-error bytes as if they had flowed. The committed-response rule is
 * documented on [reset].
 *
...
class CapturingResponseWrapper(
     ...
     * capture encoder - instead of handing back a stale tee over a stale delegate object.
     */
    override fun reset() {
        super.reset()
        capture.clear()
    ...
    // without traversing reset()/resetBuffer(); the capture must follow the buffer. Every variant is
    // overridden because HttpServletResponseWrapper delegates each one directly.
    ...
    override fun getWriter(): PrintWriter {
        ...
        // Note real.checkError() flushes, exactly as it would unwrapped.
